...
from fabric_qa.answer_ranker.train import get_batch_answers, read_qas, log_metrics, save_model, set_logger
import logging
...
def get_loss_fn(device):
    loss_fn = EnsembleRetrLoss(args.device)
    return loss_fn
...
def evaluate(model, retr_model, dataset, dataloader, eval_qas, tokenizer, opt):
    model.eval()
    retr_model.eval()
    ...
    with torch.no_grad():
        ...
        for itr, fusion_batch in tqdm(enumerate(dataloader), total=num_batch):
            ...
            scores, score_states, examples = get_score_info(model, fusion_batch)
            ...
            log_metrics(epoc, metric_rec, batch_fabric_data, retr_scores, batch_answers, time_span, total_time, 
                        (itr + 1), num_batch, loss=None) 
            
          

def train(model, retr_model, 
          train_dataset, train_qas, 
          eval_dataset, eval_dataloader, eval_qas, 
          tokenizer, 
          opt):
    ...
    for epoc in range(max_epoc):
        ...
        for itr, fusion_batch in tqdm(enumerate(train_dataloader), total=num_batch):
            ...
            if global_steps % checkpoint_steps == 0:
                ...
                save_model(args.out_dir, retr_model, epoc, tag=model_tag)
                evaluate(epoc, model, eval_data, args, qas_data=eval_qas, model_tag=model_tag)
                retr_model.train() 
...
def get_fabric_batch_data(fusion_examples, cache_dir):
    ...
    batch_fabric_examples = get_fabric_examples(batch_fabric_data, cache_dir)
    return batch_fabric_data, batch_fabric_examples 
    
def get_score_info(model, batch_data):
    with torch.no_grad():
...
def main():
    ...
    train(model, retr_model, train_dataset, eval_dataset, eval_dataloader, tokenizer, opt)
