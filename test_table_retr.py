    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(dataloader), total=num_batch):
            ...
            for k, answer_code_lst in enumerate(outputs):
                ...
                ''' 
                passage_info_lst = example['ctxs']
                table_score_dict = {}
                for p_idx, passage_info in enumerate(passage_info_lst):
                    table_id = passage_info['tag']['table_id']
                    score = passage_scores[p_idx].item()
                    if table_id not in table_score_dict:
                        table_score_dict[table_id] = []
                    
                    score_lst = table_score_dict[table_id]
                    score_lst.append(score)
                
                pred_table_id_lst = []
                pred_table_score_lst = []
                for table_id in table_score_dict:
                    pred_table_id_lst.append(table_id)
                    score_sum = np.mean(table_score_dict[table_id])
                    pred_table_score_lst.append(score_sum)
